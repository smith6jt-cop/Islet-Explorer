...
# Check for vitessceR package (optional for embedded image viewer)
VITESSCE_AVAILABLE <- requireNamespace("vitessceR", quietly = TRUE)
if (!VITESSCE_AVAILABLE) {
  message("Package 'vitessceR' not found. The embedded Vitessce viewer will be disabled.")
}

# ---- Runtime diagnostics (temporary) ----
APP_VERSION <- "cytoviewer-v1"
try({
...
# Note: Shiny automatically serves files from www/ directory
# Files in www/local_images are accessible at /local_images/ 
# Files in www/vitessce_data are accessible at /vitessce_data/
# No need to explicitly call addResourcePath for these subdirectories
...
# Check for vitessce_data annotations
vitessce_data_json_dir <- file.path("www", "vitessce_data", "json")
if (dir.exists(vitessce_data_json_dir)) {
  json_cases <- list.dirs(vitessce_data_json_dir, full.names = FALSE, recursive = FALSE)
  json_cases <- json_cases[nzchar(json_cases)]  # Remove empty strings
  cat("[SETUP] Found", length(json_cases), "case annotation directories in vitessce_data/json\n")
} else {
  cat("[SETUP] vitessce_data/json directory not found at", vitessce_data_json_dir, "\n")
}
...
discover_islet_assets <- function(image_dir = file.path("www", "local_images"),
                                  annotation_dir = file.path("www", "vitessce_data", "json")) {
  # Discover OME-TIFF images in the local_images directory
  if (!dir.exists(image_dir)) {
    warning("[ASSETS] Image directory not found: ", image_dir)
    return(data.frame())
  }
  ...
  if (!length(image_files)) {
    warning("[ASSETS] No OME-TIFF images found in: ", image_dir)
    return(data.frame())
  }

  # Helper to extract the 4-digit case ID from image filename
  # Expected pattern: {Donor_Status}_{YYYY}.ome.tiff (e.g., ND_6479.ome.tiff)
  extract_case_id <- function(fn) {
    b <- basename(fn)
    ...
    if (!nzchar(id) || is.na(suppressWarnings(as.integer(id)))) {
      # Fallback to any 3-5 digit number in the name
  ...
  # Build asset data frame - one row per image
  res <- lapply(image_files, function(img_abs) {
    img_base <- basename(img_abs)
    img_sample_id <- extract_case_id(img_base)
    
    # Derive case_id (zero-padded when numeric)
    ...
    # Extract islet token if present (for future use if images are islet-specific)
    img_islet <- stringr::str_extract(img_base, "Islet_[0-9]+")

    # Look for matching annotation JSON files in vitessce_data/json/{case_id}/
    annotation_files <- character(0)
    annotation_types <- character(0)
    if (!is.na(img_case) && nzchar(img_case) && dir.exists(annotation_dir)) {
      case_annotation_dir <- file.path(annotation_dir, img_case)
      if (dir.exists(case_annotation_dir)) {
        json_files <- list.files(case_annotation_dir, pattern = "^vitessceR_.*\\.json$", 
                                ignore.case = TRUE, full.names = FALSE)
        if (length(json_files) > 0) {
          # Extract object types from filenames (e.g., "vitessceR_Islet.json" -> "Islet")
          annotation_types <- sub("^vitessceR_", "", json_files)
          annotation_types <- sub("\\.json$", "", annotation_types, ignore.case = TRUE)
          # Build relative paths from www root
          annotation_files <- file.path("vitessce_data", "json", img_case, json_files)
        }
      }
    }
    ...
    row <- data.frame(
      ...
      image_name = img_base,
      islet_token = if (!is.na(img_islet) && nzchar(img_islet)) img_islet else NA_character_,
      num_annotations = length(annotation_files),
      stringsAsFactors = FALSE
    )
    
    # Add list columns for annotation files and types
    row$annotation_files <- I(list(annotation_files))
    row$annotation_types <- I(list(annotation_types))
...
choose_islet_asset <- function(assets, case_id, islet_key = NULL) {
  # Select the best matching image asset for a given case_id (and optional islet_key)
  if (is.null(assets) || !nrow(assets)) return(NULL)
  if (is.null(case_id) || !nzchar(case_id)) return(assets[1, , drop = FALSE])

  # Try matching by case_id (handle both raw and zero-padded formats)
  case_vec <- unique(c(case_id, suppressWarnings(sprintf("%04d", as.integer(case_id)))))
  case_vec <- case_vec[nzchar(case_vec) & !is.na(case_vec)]
  subset_assets <- assets[assets$case_id %in% case_vec, , drop = FALSE]

  # Fallback: match by grepping the case_id in the image filename
  if (!nrow(subset_assets)) {
    subset_assets <- assets[grepl(case_id, assets$image_name, fixed = TRUE), , drop = FALSE]
  }

  # If islet_key is provided and we have matches, try to narrow to islet-specific image
  # (currently most images are donor-level, but this supports future islet-specific images)
  if (!is.null(islet_key) && nzchar(islet_key) && nrow(subset_assets)) {
    match_islet <- subset_assets[!is.na(subset_assets$islet_token) & 
                                 subset_assets$islet_token == islet_key, , drop = FALSE]
    if (nrow(match_islet)) subset_assets <- match_islet
  }

  # Final fallback: grep islet_key in image name if provided
  if (!nrow(subset_assets) && !is.null(islet_key) && nzchar(islet_key)) {
    subset_assets <- assets[grepl(islet_key, assets$image_name, fixed = TRUE), , drop = FALSE]
  }
...
# Build a vitessce viewer with image and JSON annotations
build_islet_vitessce <- function(image_rel, annotation_files = NULL, annotation_types = NULL, 
                                 title = "Islet Viewer", channels = NULL, center = NULL, 
                                 case_id = NULL, use_physical_size_scaling = FALSE,
                                 base_url = NULL) {
  tryCatch({
    # Get absolute path to www directory for base_dir
    www_dir <- normalizePath("www", mustWork = TRUE)
    
    # Create config with base_dir pointing to www directory
    vc <- vitessceR::VitessceConfig$new(
      schema_version = "1.0.16", 
      name = title,
      base_dir = www_dir
    )
    dataset <- vc$add_dataset("Islet")
    
    # Get absolute path to verify file exists
    image_abs <- tryCatch({
      if (file.exists(file.path("www", image_rel))) {
        normalizePath(file.path("www", image_rel), mustWork = TRUE)
      } else if (file.exists(image_rel)) {
        normalizePath(image_rel, mustWork = TRUE)
      } else {
        stop("Image file not found: ", image_rel)
      }
    }, error = function(e) {
      warning("[VITESSCE] Could not resolve image path: ", image_rel, " - ", e$message)
      NULL
    })
    
    if (is.null(image_abs)) {
      stop("Could not resolve image path")
    }
    
    message(sprintf("[VITESSCE] Primary image (absolute): %s", image_abs))
    message(sprintf("[VITESSCE] Image exists: %s", file.exists(image_abs)))
    message(sprintf("[VITESSCE] Image readable: %s", file.access(image_abs, 4) == 0))
    message(sprintf("[VITESSCE] Image size: %.2f GB", file.size(image_abs) / 1e9))
    base_dims <- get_image_dims(image_abs)
    if (!is.null(base_dims)) {
      message(sprintf("[VITESSCE] Base dimensions: %dx%d px", base_dims$width, base_dims$height))
    } else {
      message("[VITESSCE] Base dimensions: unknown (magick not available)")
    }
    
    # CRITICAL: When using base_dir, img_path should be RELATIVE to base_dir
    # Since base_dir = www/, and image is at www/local_images/file.tiff,
    # the relative path should be just "local_images/file.tiff"
    image_path_relative <- image_rel
    message(sprintf("[VITESSCE] Image path (relative to base_dir): %s", image_path_relative))
    
    # Build image wrappers list with paths RELATIVE to base_dir
    primary_wrapper <- vitessceR::OmeTiffWrapper$new(
      img_path = image_path_relative,  # Relative to base_dir
      name = "Multiplex",
      is_bitmask = FALSE
    )
    # Set unique identifier for route stability
    if ("set_uid" %in% names(primary_wrapper)) {
      primary_wrapper$set_uid(sprintf("wr_%03d", wrapper_idx))
    } else if ("uid" %in% names(primary_wrapper)) {
      primary_wrapper$uid <- sprintf("wr_%03d", wrapper_idx)
    }
    
    image_wrappers <- list(primary_wrapper)
    
    # Add multi-image wrapper to dataset first
    message(sprintf("[VITESSCE] Scaling mode: %s", if (isTRUE(use_physical_size_scaling)) "physical-size (µm)" else "pixel-based"))
    if (isTRUE(use_physical_size_scaling) && (is.null(image_abs) || is.null(base_dims))) {
      message("[VITESSCE] Note: physical-size scaling requested but base pixel size metadata is unavailable in this context; visual alignment may still rely on pixel sizes.")
    }
    dataset$add_object(
      vitessceR::MultiImageWrapper$new(
        image_wrappers = image_wrappers,
        use_physical_size_scaling = isTRUE(use_physical_size_scaling)
      )
    )
    
    # Add JSON annotations if available - try adding as separate dataset objects
    if (!is.null(annotation_files) && length(annotation_files) > 0) {
      message(sprintf("[VITESSCE] Attempting to add %d JSON annotation file(s)", length(annotation_files)))
      
      # For now, skip annotations as vitessceR v0.99.0 doesn't have proper support
      # The ObsSegmentationsWrapper class doesn't exist in this version
      # TODO: Upgrade vitessceR or implement custom JSON handler
      message("[VITESSCE] Note: JSON annotations found but skipped (vitessceR v0.99.0 lacks ObsSegmentationsWrapper)")
      message("[VITESSCE] Available annotations: ", paste(annotation_types %||% "unknown", collapse=", "))
    } else {
      message("[VITESSCE] No JSON annotations available")
    }
    
    # Add multi-image wrapper to dataset
    message(sprintf("[VITESSCE] Scaling mode: %s", if (isTRUE(use_physical_size_scaling)) "physical-size (µm)" else "pixel-based"))
    if (isTRUE(use_physical_size_scaling) && (is.null(image_abs) || is.null(base_dims))) {
      message("[VITESSCE] Note: physical-size scaling requested but base pixel size metadata is unavailable in this context; visual alignment may still rely on pixel sizes.")
    }
  ...
  # Create views: spatial view (9) + layer controller (3) — removed STATUS per request
  spatial <- vc$add_view(dataset, vitessceR::Component$SPATIAL, x = 0, y = 0, w = 9, h = 12)
  layer_ctrl <- vc$add_view(dataset, vitessceR::Component$LAYER_CONTROLLER, x = 9, y = 0, w = 3, h = 12)

    # If center provided, always convert µm to px (robust even when layers disagree on physical-size)
    if (!is.null(center) && !is.null(image_abs)) {
      px_size <- get_pixel_size_um(image_abs)
      if (!is.null(px_size)) {
        cx <- center$x / px_size$x_um_per_px
        cy <- center$y / px_size$y_um_per_px
        message(sprintf("[VITESSCE] Applying initial center/zoom (px): x=%.1f y=%.1f z=%s (µm/px: %.4f, %.4f)",
                        cx, cy, ifelse(is.null(center$zoom), "NA", sprintf("%.2f", center$zoom)), px_size$x_um_per_px, px_size$y_um_per_px))
        coord <- vc$add_coordination(c("spatialTargetX", "spatialTargetY", "spatialZoom"))
        coord[[1]]$set_value(cx)
        coord[[2]]$set_value(cy)
        if (!is.null(center$zoom)) coord[[3]]$set_value(center$zoom)
        spatial$use_coordination(coord)
      } else {
        message("[VITESSCE] Pixel centering skipped: base PhysicalSizeX/Y unavailable; user can pan/zoom manually.")
      }
    }
    # Note: Do not add non-standard coordination keys like 'usePhysicalSizeScaling'.
    # Scaling mode is controlled via MultiImageWrapper$use_physical_size_scaling above.
    
    # Do not set spatialTarget when using pixel-based alignment; avoid unit mismatches across layers
    
  # Layout with spatial view and layer controller only
  vc$layout(vitessceR::hconcat(spatial, layer_ctrl))
    
    # DEBUG: Check VitessceConfig internals before creating widget
    message("[VITESSCE DEBUG] Checking VitessceConfig before widget creation...")
    tryCatch({
      if (!is.null(vc$to_list)) {
        config_list <- vc$to_list()
        message(sprintf("[VITESSCE DEBUG] Config has %d dataset(s) in to_list()", 
                       length(config_list$datasets %||% list())))
      }
    }, error = function(e) {
      message(sprintf("[VITESSCE DEBUG] Could not inspect config: %s", e$message))
    })
    
    # Create widget with serve=FALSE and base_url for Shiny integration
    # This tells vitessceR to NOT start its own plumber server
    # Instead, Shiny will serve files from the www/ directory
    widget <- vc$widget(
      theme = "light", 
      width = "100%", 
      height = "80vh",
    ...
    # Post-process: reduce default visible segmentation layers and inject channel names when possible
    try({
      cfg <- widget$x$config
      # Inject channelNames into base image entries when available
      if (!is.null(channels) && length(channels) > 0 && !is.null(cfg$datasets)) {
        for (ds_name in names(cfg$datasets)) {
          ds <- cfg$datasets[[ds_name]]
          if (!is.null(ds$files)) {
            for (fi in seq_along(ds$files)) {
              f <- ds$files[[fi]]
              if (!is.null(f$options) && !is.null(f$options$images)) {
                for (im in seq_along(f$options$images)) {
                  img <- f$options$images[[im]]
                  if (is.null(img$name) || identical(img$name, "Multiplex") || im == 1L) {
                    f$options$images[[im]]$channelNames <- as.list(as.character(channels))
                  }
                }
                ds$files[[fi]] <- f
              }
          ...
          cfg$datasets[[ds_name]] <- ds
        }
      }
      # Turn off all but first two raster layers by default, if coordination exists
      if (!is.null(cfg$coordinationSpace) && !is.null(cfg$coordinationSpace$spatialRasterLayers)) {
        scs <- cfg$coordinationSpace$spatialRasterLayers
        for (scope in names(scs)) {
          layers <- scs[[scope]]
          if (is.list(layers) && length(layers) > 0) {
            for (i in seq_along(layers)) {
              if (is.null(layers[[i]]$visible)) layers[[i]]$visible <- TRUE
              if (i > 2L) layers[[i]]$visible <- FALSE
            }
            scs[[scope]] <- layers
          }
        }
        cfg$coordinationSpace$spatialRasterLayers <- scs
      }
      widget$x$config <- cfg
    }, silent = TRUE)

    # Only apply the legacy 'dataset' coordination removal for older vitessceR versions
    # Newer versions (>= 0.1.0) should not require this and removing it can break the config
    do_dataset_fix <- FALSE
    vv <- tryCatch(packageVersion("vitessceR"), error = function(e) NULL)
    if (!is.null(vv)) do_dataset_fix <- isTRUE(vv < "0.1.0")
    if (isTRUE(do_dataset_fix)) {
      if (!is.null(widget$x$config$coordinationSpace) && "dataset" %in% names(widget$x$config$coordinationSpace)) {
        widget$x$config$coordinationSpace$dataset <- NULL
        message("[VITESSCE FIX] Removed invalid 'dataset' coordination from coordinationSpace")
      }
      if (!is.null(widget$x$config$layout)) {
        for (i in seq_along(widget$x$config$layout)) {
          if (!is.null(widget$x$config$layout[[i]]$coordinationScopes)) {
            if ("dataset" %in% names(widget$x$config$layout[[i]]$coordinationScopes)) {
              widget$x$config$layout[[i]]$coordinationScopes$dataset <- NULL
              message("[VITESSCE FIX] Removed 'dataset' from layout component coordinationScopes")
            }
          }
        }
      }
    ...
    # Set element ID if case_id provided (for debugging)
    if (!is.null(case_id) && is.null(widget$elementId)) {
      widget$elementId <- paste0("vitessce_", gsub("\\W", "_", case_id))
    }
    
    widget
    
  }, error = function(e) {
    message(sprintf("[VITESSCE ERROR] %s", e$message))
    message(sprintf("[VITESSCE ERROR] image_rel=%s, seg_rel=%s", image_rel, seg_rel %||% "NULL"))
    
    # Return minimal fallback config using consistent method
    vc_fallback <- vitessceR::VitessceConfig$new(schema_version = "1.0.16", name = "Error")
    vc_fallback$widget(theme = "light", height = 800)
  })
}
    ...
    column(width = 10.6, class = "equal-height-panel main-content-panel",
        ...
        tabPanel("Plot", 
                 fluidRow(
                   ...
                   column(4,
                     div(class = "card", style = "margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; height: calc(100vh - 100px);",
                       h5("Islet Size Distribution", style = "margin-top: 0; color: #333;"),
                       plotlyOutput("plt", height = 500),
                       br(),
                   ...
                   column(4,
                     div(class = "card", style = "margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; height: calc(100vh - 100px); display: flex; flex-direction: column;",
                   ...
                   column(4,
                     div(class = "card ai-chat-panel", style = "margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; height: calc(100vh - 100px); display: flex; flex-direction: column;",
                       div(
          ...
          tagList(
            fluidRow(
              column(6,
                radioButtons(
                  "pseudotime_method",
                  "Pseudotime Calculation Method:",
                  choices = c("PAGA (Python/Scanpy)" = "paga", "Lamian (R/TSCAN)" = "lamian"),
                  selected = "paga",
                  inline = TRUE
                )
              ),
            ...
            br(),
            # Embedded vitessce viewer for selected islet
            fluidRow(
              column(12,
                h3("Selected Islet Viewer", style = "font-weight: bold; margin-top: 20px; margin-bottom: 15px;"),
                uiOutput("selected_islet_info"),
                # Manual vitessce iframe viewer
...
server <- function(input, output, session) {
  ...
  traj_with_pseudotime <- reactive({
    ...
    method <- input$pseudotime_method
    if (is.null(method)) method <- "paga"  # default
    
    obs <- tr$obs
    ad <- tr$adata
    
    # Compute pseudotime based on selected method
    if (method == "lamian" && !is.null(ad)) {
      # Compute Lamian pseudotime
      lamian_pt <- compute_lamian_pseudotime(ad)
      if (!all(is.na(lamian_pt))) {
        obs$pseudotime <- lamian_pt
      }
    } else {
      # Use PAGA (dpt_pseudotime)
  ...
  output$traj_status <- renderUI({
    ...
    method <- input$pseudotime_method %||% "paga"
    method_label <- if (method == "lamian") "Lamian (R/TSCAN)" else "PAGA (Python/Scanpy)"
  ...
  output$traj_scatter <- renderPlotly({
    ...
    method <- input$pseudotime_method %||% "paga"
    x_label <- if (method == "lamian") {
      "Pseudotime (Lamian/TSCAN trajectory, INS-rooted)"
    } else {
      "Pseudotime (PAGA trajectory, INS-rooted)"
    }
    ...
    if ("combined_islet_id" %in% colnames(df) && "case_id" %in% colnames(df)) {
      ...
      # Calculate appropriate tolerance based on jitter amount
      pt_range <- diff(range(df$pt, na.rm = TRUE))
      value_range <- diff(range(df$value, na.rm = TRUE))
      tolerance_x <- pt_range * 0.015  # 1.5% of x range (jitter is 1% + margin)
      tolerance_y <- value_range * 0.025  # 2.5% of y range (jitter is 2% + margin)
        ...
        if (!is.null(p$x$data[[i]]$type) && p$x$data[[i]]$type == "scatter") {
          ...
          if (length(trace_x) > 0 && length(trace_y) > 0) {
            ...
            for (j in seq_along(trace_x)) {
              ...
              matches <- which(
                abs(df$pt - trace_x[j]) < tolerance_x &
                abs(df$value - trace_y[j]) < tolerance_y
              )
              ...
              if (length(matches) > 0) {
                ...
                distances <- sqrt(((df$pt - trace_x[j])/pt_range)^2 +
                                ((df$value - trace_y[j])/value_range)^2)
                idx <- which.min(distances)
                if (distances[idx] < 0.05) {  # Within 5% normalized distance
                  trace_customdata[j, ] <- c(
                    as.character(df$case_id[idx]),
                    as.character(df$islet_key[idx]),
                    as.character(df$combined_islet_id[idx]),
            ...
            cat("[PLOT] Trace", i, ":", n_matched, "/", length(trace_x), "points matched with customdata\n")
          }
        }
      }
  ...
  # FIXED: Enhanced click handler with better debugging and UI feedback
  observe({
    click_data <- event_data("plotly_click", source = "traj_scatter")
    if (is.null(click_data)) return()
    
    # Visual notification that click was detected
    showNotification("Point clicked! Loading islet viewer...", type = "message", duration = 2)
    
    cat("[CLICK] ========== NEW CLICK EVENT ==========\n")
    cat("[CLICK] Timestamp:", as.character(Sys.time()), "\n")
    cat("[CLICK] Point number:", click_data$pointNumber, "\n")
    cat("[CLICK] Coordinates: x =", click_data$x, ", y =", click_data$y, "\n")
    flush.console()
    
    # Debug customdata
    if (!is.null(click_data$customdata)) {
      cat("[CLICK] customdata available:", length(click_data$customdata), "elements\n")
      cat("[CLICK] customdata content:", paste(click_data$customdata, collapse=" | "), "\n")
    } else {
      cat("[CLICK] WARNING: No customdata available\n")
    }
    flush.console()
    
    # Method 1: Try customdata (most reliable)
    islet_info <- NULL
    if (!is.null(click_data$customdata) && length(click_data$customdata) >= 4) {
      islet_info <- list(
        case_id = as.character(click_data$customdata[1]),
        islet_key = as.character(click_data$customdata[2]),
        combined_islet_id = as.character(click_data$customdata[3]),
        donor_status = as.character(click_data$customdata[4])
      )
      cat("[CLICK] Method: customdata\n")
    } else if (exists("traj_coord_lookup")) {
      # Method 2: Coordinate-based lookup
      tolerance <- 0.01  # Increased tolerance for floating point comparison
      ...
      if (length(matching_rows) > 0) {
        idx <- matching_rows[1]
        islet_info <- list(
          case_id = as.character(traj_coord_lookup$case_id[idx]),
          islet_key = as.character(traj_coord_lookup$islet_key[idx]),
          combined_islet_id = as.character(traj_coord_lookup$combined_islet_id[idx]),
          donor_status = as.character(traj_coord_lookup$donor_status[idx])
        )
        cat("[CLICK] Method: coordinate match (idx=", idx, ")\n")
      } else {
        # Nearest neighbor fallback
        distances <- sqrt((traj_coord_lookup$pt - click_data$x)^2 + 
                         (traj_coord_lookup$value - click_data$y)^2)
        ...
        if (min_distance < 0.1) {  # Reasonable threshold
          ...
          cat("[CLICK] Method: nearest neighbor (distance=", min_distance, ")\n")
        }
      }
    }
    
    if (!is.null(islet_info)) {
      cat("[CLICK] Selected: case=", islet_info$case_id,
          " islet=", islet_info$islet_key,
          " status=", islet_info$donor_status, "\n")
      flush.console()

      # Add spatial coordinates if available
      annot <- get_islet_annotations(islet_info$case_id, islet_info$islet_key)
      if (!is.null(annot)) {
        islet_info$centroid_x_um <- annot$centroid_x
        islet_info$centroid_y_um <- annot$centroid_y
        islet_info$has_spatial <- TRUE
        islet_info$annotation_classes <- annot$classes
        cat("[CLICK] Spatial coords: (", annot$centroid_x, ", ", annot$centroid_y, ")\n")
        cat("[CLICK] Annotation classes:", paste(annot$classes, collapse = ", "), "\n")
      } else {
        islet_info$has_spatial <- FALSE
        cat("[CLICK] No spatial coordinates found\n")
      }
      flush.console()

      # Update reactive value to trigger viewer update
      selected_islet(islet_info)
      cat("[CLICK] ✓ Islet selection updated - viewer should reload\n")
      cat("[CLICK] ========================================\n\n")
      flush.console()
    } else {
      cat("[CLICK] ❌ Failed to identify islet from click\n")
      cat("[CLICK] ========================================\n\n")
      flush.console()
    }
  })
  
  # Display selected islet information
  output$selected_islet_info <- renderUI({
    islet <- selected_islet()
    
    if (is.null(islet)) {
      return(tags$div(
        style = "padding: 10px; background: #f0f0f0; border-radius: 4px; margin-bottom: 10px;",
        tags$em("Click a point in the trajectory plot above to view the islet image")
      ))
    }
    
    # Build info display
    info_parts <- c(
      paste0("📍 ", islet$combined_islet_id),
      paste0("Case: ", islet$case_id),
      paste0("Status: ", islet$donor_status)
    )
    
    if (isTRUE(islet$has_spatial)) {
      info_parts <- c(
        info_parts,
        sprintf("Coords: (%.1f, %.1f) µm", islet$centroid_x_um, islet$centroid_y_um)
      )
      
      if (!is.null(islet$annotation_classes)) {
        info_parts <- c(
          info_parts,
          paste0("Classes: ", paste(islet$annotation_classes, collapse = ", "))
        )
      }
    }
    
    tags$div(
      style = "padding: 10px; background: #e3f2fd; border-left: 4px solid #2c5aa0; border-radius: 4px; margin-bottom: 10px;",
      tags$strong("Selected Islet: "),
      tags$span(paste(info_parts, collapse = " | "))
    )
  })
  
  # Fallback UI when vitessceR not installed
  if (!VITESSCE_AVAILABLE) {
    output$traj_vitessce_fallback <- renderUI({
      tags$div(
        style = "background:#fff3cd; padding:20px; border-radius:6px; border-left:4px solid #ffc107; margin:20px;",
        tags$h4("⚠ vitessceR Package Not Installed", style = "margin-top:0;"),
        tags$p("The vitessceR package is required to display the interactive islet image viewer."),
        tags$p(tags$strong("To install vitessceR, run one of these commands in R:")),
        tags$pre(
          style = "background:#f8f8f8; padding:10px; border-radius:4px; overflow-x:auto;",
          "# Option 1: Install from GitHub (recommended)\nremotes::install_github('vitessce/vitessceR')\n\n# Option 2: Install from CRAN (if available)\ninstall.packages('vitessceR')"
        ),
        tags$p(style = "margin-top:15px; color:#856404;",
               tags$em("After installation, restart the Shiny app to enable the viewer."))
      )
    })
  }
  
  # Discover available image and annotation assets at startup
  vitessce_assets <- reactive({
    assets_list <- discover_islet_assets(
      image_dir = file.path("www", "local_images"),
      annotation_dir = file.path("www", "vitessce_data", "json")
    )
    
    if (nrow(assets_list) > 0) {
      cat(sprintf("[ASSETS] Found %d image(s) with annotations\n", nrow(assets_list)))
      cat(sprintf("[ASSETS] First case_id: %s\n", assets_list$case_id[1]))
      if (assets_list$num_annotations[1] > 0) {
        cat(sprintf("[ASSETS] First case has %d annotation file(s)\n", assets_list$num_annotations[1]))
      }
    } else {
      cat("[ASSETS] No assets found\n")
    }
    
    assets_list
  })

  # DIRECT VITESSCE.JS APPROACH: Build config JSON manually and embed JavaScript widget
  # This bypasses vitessceR's broken R-to-JS serialization in v0.99.0
  
  # Store vitessce config server-side to avoid URL length limits
  vitessce_config <- reactiveVal(NULL)
  
  output$traj_vitessce_viewer <- renderUI({
    # Force reactivity on selected islet and assets
    islet <- selected_islet()
    assets <- vitessce_assets()

    cat("[VITESSCE MANUAL] Render called\n")
    
    # Placeholder when no islet selected
    if (is.null(islet)) {
      return(div(
        style = "padding: 20px; text-align: center; color: #666; background: #f8f9fa; border-radius: 5px; margin: 10px;",
        h4("Click a point in the trajectory plot to view the islet"),
        p("The viewer will display the donor tissue image with segmentation annotations overlaid.")
      ))
    }

    # Check assets
    if (is.null(assets) || !nrow(assets)) {
      return(div(
        style = "padding: 20px; text-align: center; color: #ff6b6b; background: #ffe0e0; border-radius: 5px; margin: 10px;",
        h4("No OME-TIFF images found"),
        p("Please ensure images are available in www/local_images/")
      ))
    }

    # Find matching asset
    asset <- choose_islet_asset(assets, case_id = islet$case_id, islet_key = islet$islet_key)
    
    if (is.null(asset)) {
      return(div(
        style = "padding: 20px; text-align: center; color: #ff6b6b; background: #ffe0e0; border-radius: 5px; margin: 10px;",
        h4(paste("No image available for case", islet$case_id)),
        p("This donor does not have an OME-TIFF image or annotations in the data directory.")
      ))
    }

    # Get annotation files
    annotation_files <- if ("annotation_files" %in% names(asset) && !is.null(asset$annotation_files[[1]])) {
      asset$annotation_files[[1]]
    } else {
      character(0)
    }
    
    annotation_types <- if ("annotation_types" %in% names(asset) && !is.null(asset$annotation_types[[1]])) {
      asset$annotation_types[[1]]
    } else {
      character(0)
    }

    cat(sprintf("[VITESSCE MANUAL] Case %s: image=%s, annotations=%d\n", 
                islet$case_id, basename(asset$image_rel), length(annotation_files)))

    # Build Vitessce config JSON manually
    # Image URL (served from Shiny www/)
    image_url <- paste0("/local_images/", basename(asset$image_rel))
    ...
    # Add annotation files as obsSegmentations
    for (i in seq_along(annotation_files)) {
      ann_url <- paste0("/vitessce_data/json/", islet$case_id, "/", basename(annotation_files[i]))
      ann_type <- if (i <= length(annotation_types)) annotation_types[i] else "annotation"
      
      files_list <- c(files_list, list(list(
        fileType = "obsSegmentations.json",
        url = ann_url,
        coordinationValues = list(
          obsType = ann_type,
          fileUid = paste0("ann_", i-1)
        )
      )))
    }
    
    # Calculate center in pixels if we have centroid
    center_x <- 5000  # default center
    center_y <- 5000
    zoom_level <- -5
    
    if (!is.null(islet$centroid_x_um) && !is.null(islet$centroid_y_um)) {
      # Try to get pixel size to convert µm to pixels
      image_abs <- file.path("www", asset$image_rel)
      px_size <- get_pixel_size_um(image_abs)
      if (!is.null(px_size)) {
        center_x <- islet$centroid_x_um / px_size$x_um_per_px
        center_y <- islet$centroid_y_um / px_size$y_um_per_px
        zoom_level <- -1  # Closer zoom when we know the location
        cat(sprintf("[VITESSCE MANUAL] Center: %.1f,%.1f px (from %.1f,%.1f µm)\n",
                    center_x, center_y, islet$centroid_x_um, islet$centroid_y_um))
      }
    }
    
    # Build minimal Vitessce config
    config <- list(
      version = "1.0.16",
      name = sprintf("Case %s", islet$case_id),
      description = paste(islet$islet_key %||% "", islet$donor_status %||% ""),
      datasets = list(
        list(
          uid = "A",
          name = basename(asset$image_rel),
          files = files_list
        )
      ),
      coordinationSpace = list(
        spatialTargetX = list(A = center_x),
        spatialTargetY = list(A = center_y),
        spatialZoom = list(A = zoom_level)
      ),
      layout = list(
        list(
          component = "spatial",
    ...
    vitessce_config(config)
    
    # Create unique viewer ID
    viewer_id <- paste0("vitessce_viewer_", gsub("[^a-zA-Z0-9]", "_", islet$case_id))
    
    cat(sprintf("[VITESSCE MANUAL] Config stored, creating viewer div\n"))
    
    # Embed Vitessce viewer using simplified loader
    config_json <- jsonlite::toJSON(config, auto_unbox = TRUE)
    
    tagList(
      # Import map to resolve bare module specifiers (must come first)
      tags$script(
        type = "importmap",
        HTML('
{
    ...
    "react-dom/client": "https://esm.sh/react-dom@18/client"
      ...
      tags$div(
        ...
        tags$div(
          ...
          "Click detected - initializing viewer..."
        )
      ),
      # Load the loader script (IIFE, executes immediately)
      tags$script(type = "module", src = "./vitessce_loader.js"),
      # Then call it after a brief delay to ensure loader is ready
      tags$script(HTML(sprintf(
        ...
        setTimeout(async function() {
          ...
          console.log('[SHINY] Initializing viewer for:', containerId);
          
          try {
            if (window.VitessceLoader) {
              await window.VitessceLoader.createViewer(containerId, config);
            } else {
              console.error('[SHINY] VitessceLoader not found - retrying...');
              ...
              setTimeout(async function() {
                if (window.VitessceLoader) {
                  ...
                  document.getElementById(containerId).innerHTML = 
                    '<div style=\"padding:20px;color:red;\">VitessceLoader failed to load</div>';
                }
              }, 1000);
            }
          } catch (error) {
            console.error('[SHINY] Error:', error);
            document.getElementById(containerId).innerHTML = 
  ...
  plot_df <- reactive({
    ...
    plot_outliers <<- if (nrow(outliers) > 0) {
      data.frame(
        Mode = rep(input$mode %||% "Unknown", nrow(outliers)),
        Selection = rep(input$which %||% "Unknown", nrow(outliers)),
    ...
    if (isTRUE(input$show_points)) {
      ...
      if (color_by == "donor_id") {
        ...
        if ("Case ID" %in% colnames(raw_normal)) {
          ...
          n_donors <- length(levels(raw_normal$donor_id))
          if (n_donors <= 12) {
            donor_colors <- RColorBrewer::brewer.pal(min(12, max(3, n_donors)), "Paired")
            names(donor_colors) <- levels(raw_normal$donor_id)
          } else {
            donor_colors <- rainbow(n_donors, s = 0.8, v = 0.8)
            names(donor_colors) <- levels(raw_normal$donor_id)
          }
          p <- p +
            ...
            scale_color_manual(values = c(color_map, donor_colors), 
                             breaks = c(grp_levels, levels(raw_normal$donor_id)),
                             name = "Color")
        } else {
  ...
  output$dist <- renderPlotly({
    ...
    # ALWAYS group and color violin/box by donor_status
    g <- ggplot(rdf, aes(x = donor_status, y = value, fill = donor_status))
    
    if (!is.null(input$dist_type) && input$dist_type == "Box") {
      # NUCLEAR OPTION: combine ALL outlier suppression methods
      g <- g + geom_boxplot(
        outlier.shape = NA,
        outlier.size = 0,
        outlier.colour = NA,
    ...
    # Apply standard donor status colors to violin/box - NO LEGEND
    g <- g + scale_fill_manual(values = c("ND" = "#1f77b4", "Aab+" = "#ff7f0e", "T1D" = "#d62728"), guide = "none")
    
    # Add points with color scheme based on selector
    if (isTRUE(input$dist_show_points)) {
      if (color_by == "donor_id" && "donor_id" %in% colnames(rdf)) {
        # Color points by donor ID with distinct colors
        n_donors <- length(levels(rdf$donor_id))
        cat(sprintf("[DIST] Coloring by donor_id: %d donors\n", n_donors))
        cat(sprintf("[DIST] Donor IDs: %s\n", paste(unique(rdf$donor_id), collapse=", ")))
        
        if (n_donors <= 12) {
          donor_colors <- RColorBrewer::brewer.pal(min(12, max(3, n_donors)), "Paired")
        } else {
          donor_colors <- rainbow(n_donors, s = 0.9, v = 0.9)
        }
        names(donor_colors) <- levels(rdf$donor_id)
        
        cat(sprintf("[DIST] Assigned colors: %s\n", paste(names(donor_colors), "=", donor_colors, collapse="; ")))
        ...
        g <- g + geom_jitter(aes(color = donor_id), width = 0.18,
                           ...
                           stroke = 0) +
          scale_color_manual(values = donor_colors, guide = "none")  # NO LEGEND
      } else {
        # Color points by donor_status (match violin/box colors)
        cat("[DIST] Coloring by donor_status\n")
    ...
    g <- g + labs(x = "Donor Status", y = ylab, title = "Distribution across donor groups") +
      theme_minimal(base_size = 14) +
      theme(legend.position = "none")  # NO LEGEND for Distribution plot
    
    # Apply log scale to y-axis if selected (use dist-specific control)
    ...
    # Convert to plotly with showlegend=FALSE to completely remove legend
    p <- ggplotly(g, tooltip = c("x", "y"))
    
    # AGGRESSIVELY HIDE boxplot outliers in plotly
    if (!is.null(input$dist_type) && input$dist_type == "Box") {
      for (i in seq_along(p$x$data)) {
        if (!is.null(p$x$data[[i]]$type) && p$x$data[[i]]$type == "box") {
          # Try multiple ways to hide outliers
          ...
          p$x$data[[i]]$boxpoints <- FALSE  # This should disable all points
        }
      }
    }
    
    # Hide all legend entries in plotly
