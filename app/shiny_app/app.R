...
# Build a working vitessce viewer with proper segmentation layer control
build_islet_vitessce <- function(image_rel, seg_rel = NULL, title = "Islet Viewer",
                                 channels = NULL, center = NULL, case_id = NULL,
                                 seg_labels = NULL, use_physical_size_scaling = FALSE) {
  tryCatch({
    # Create config
    vc <- vitessceR::VitessceConfig$new(schema_version = "1.0.16", name = title)
    dataset <- vc$add_dataset("Islet")
    
    # Convert relative paths to absolute paths for proper loading
    # The key fix: use img_path with full normalized paths, not img_url with relative paths
    image_abs <- tryCatch({
      # Try to resolve relative path from www directory
      ...
      image_rel  # fallback to relative path
    })
    
    message(sprintf("[VITESSCE] Primary image: %s", image_abs))
    ...
    # Build image wrappers list with absolute paths
    # CRITICAL: Set unique UIDs for stable route registration
    wrapper_idx <- 1L
    ...
    wrapper_idx <- wrapper_idx + 1L
    
  # Add segmentation layer(s) if available (vector input supported)
    if (!is.null(seg_rel) && length(seg_rel) > 0) {
      seg_vec <- as.character(seg_rel)
      lab_vec <- if (!is.null(seg_labels) && length(seg_labels) == length(seg_vec)) as.character(seg_labels) else rep("Segmentation", length(seg_vec))
      for (i in seq_along(seg_vec)) {
        sr <- seg_vec[[i]]
        if (!nzchar(sr)) next
        seg_abs <- tryCatch({
          if (file.exists(file.path("www", sr))) {
            normalizePath(file.path("www", sr), mustWork = TRUE)
          } else if (file.exists(sr)) {
            normalizePath(sr, mustWork = TRUE)
          } else {
            stop("Segmentation file not found: ", sr)
          }
        }, error = function(e) {
          warning("[VITESSCE] Could not resolve segmentation path: ", sr, " - ", e$message)
          NULL
        })
        if (is.null(seg_abs)) next
        # Optional: filter segmentation if dimensions don't match base (prevents obvious mis-scale)
        if (!is.null(base_dims)) {
          seg_dims <- get_image_dims(seg_abs)
          if (!is.null(seg_dims)) {
            if (seg_dims$width != base_dims$width || seg_dims$height != base_dims$height) {
              message(sprintf("[VITESSCE] Skipping segmentation (dimension mismatch): %s (%dx%d vs base %dx%d)",
                              seg_abs, seg_dims$width, seg_dims$height, base_dims$width, base_dims$height))
              next
            }
          }
        }
        message(sprintf("[VITESSCE] Adding segmentation layer: %s", seg_abs))
        seg_wrapper <- vitessceR::OmeTiffWrapper$new(
          img_path = seg_abs,
          name = lab_vec[[i]],
          is_bitmask = TRUE
        )
        # Debug: log dimensions if available (will show once routes are active)
        try({
          if (!is.null(primary_wrapper$img_path) && !is.null(seg_wrapper$img_path)) {
            message(sprintf("[VITESSCE DEBUG] base=%s seg=%s", primary_wrapper$img_path, seg_wrapper$img_path))
          }
        }, silent = TRUE)
        if ("set_uid" %in% names(seg_wrapper)) {
          seg_wrapper$set_uid(sprintf("wr_%03d", wrapper_idx))
        } else if ("uid" %in% names(seg_wrapper)) {
          seg_wrapper$uid <- sprintf("wr_%03d", wrapper_idx)
        }
        image_wrappers <- append(image_wrappers, list(seg_wrapper))
        wrapper_idx <- wrapper_idx + 1L
      }
    }
    ...
    if (isTRUE(use_physical_size_scaling) && is.null(base_dims)) {
  ...
  vc$layout(vitessceR::hconcat(spatial, layer_ctrl))
    ...
    # Use the standard widget method to ensure proper Shiny integration
    # This is CRITICAL - it registers the HTTP routes for serving image tiles
    widget <- vc$widget(theme = "light", width = "100%", height = "80vh")
    ...
    vitessceR::vitessce_widget(vc_fallback, theme = "light", height = 800)
  })
}
          ...
          tagList(
            ...
            fluidRow(
              column(12,
                ...
                div(id = "vitessce-viewer-container",
                  if (VITESSCE_AVAILABLE) {
                    vitessceR::vitessce_output("traj_vitessce_viewer", height = 600)
                  } else {
                    uiOutput("traj_vitessce_fallback")
                  }
                )
              )
            ),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(islet_info)) {
      ...
      cat("[CLICK] Failed to identify islet\n")
    }
  })
  ...
  # Discover available image and segmentation assets at startup
  vitessce_assets <- reactive({
    ...
    if (nrow(assets_list) > 0) {
      cat(sprintf("[ASSETS] Found %d image(s) with potential segmentation pairs\n", nrow(assets_list)))
  ...
  # Render vitessce viewer for trajectory tab
  if (VITESSCE_AVAILABLE) {
    output$traj_vitessce_viewer <- vitessceR::render_vitessce({
      islet <- selected_islet()
      assets <- vitessce_assets()
      
      cat("\n[VITESSCE RENDER] ===== START =====\n")
      cat(sprintf("[VITESSCE] Islet selected: %s\n", !is.null(islet)))
      cat(sprintf("[VITESSCE] Assets available: %d\n", if(!is.null(assets)) nrow(assets) else 0))

      # Handle no islet selected
      if (is.null(islet)) {
        cat("[VITESSCE] No islet selected, showing placeholder\n")
        cfg <- vitessceR::VitessceConfig$new(
          schema_version = "1.0.16",
          name = "Click a point in the trajectory plot to view islet"
        )
        return(vitessceR::vitessce_widget(cfg, theme = "light", height = 600, element_id = "vitessce_placeholder"))
      }

      # Handle no assets available
      if (is.null(assets) || !nrow(assets)) {
        cat("[VITESSCE] No assets available\n")
        cfg <- vitessceR::VitessceConfig$new(
          schema_version = "1.0.16",
          name = "No OME-TIFF images found in www/local_images"
        )
        return(vitessceR::vitessce_widget(cfg, theme = "light", height = 600, element_id = "vitessce_no_assets"))
      }

      # Find matching asset for the selected islet
      cat(sprintf("[VITESSCE] Looking for case_id=%s, islet_key=%s\n", 
                  islet$case_id, islet$islet_key %||% "NULL"))
      
      asset <- choose_islet_asset(assets, case_id = islet$case_id, islet_key = islet$islet_key)
      
      if (is.null(asset)) {
        cat(sprintf("[VITESSCE] No matching asset found for case %s\n", islet$case_id))
        cfg <- vitessceR::VitessceConfig$new(
          schema_version = "1.0.16",
          name = sprintf("No image found for case %s", islet$case_id)
        )
        return(vitessceR::vitessce_widget(cfg, theme = "light", height = 600, element_id = "vitessce_no_match"))
      }

      cat(sprintf("[VITESSCE] Found asset: %s\n", asset$image_name))
      cat(sprintf("[VITESSCE] Image path: %s\n", asset$image_rel))
      
      # Get segmentation path(s)
      seg_rel <- NULL
      seg_labels <- NULL
      if ("seg_rel_list" %in% names(asset) && length(asset$seg_rel_list[[1]]) > 0) {
        seg_rel <- asset$seg_rel_list[[1]]
        seg_labels <- if ("seg_label_list" %in% names(asset)) asset$seg_label_list[[1]] else NULL
        cat(sprintf("[VITESSCE] %d segmentation layer(s) available\n", length(seg_rel)))
      } else if (!is.na(asset$seg_rel) && nzchar(asset$seg_rel)) {
        seg_rel <- asset$seg_rel
        cat(sprintf("[VITESSCE] Segmentation path: %s\n", seg_rel))
      } else {
        cat("[VITESSCE] No segmentation layer available\n")
      }
      ...
      if (!is.null(islet$centroid_x_um) && !is.null(islet$centroid_y_um)) {
        center <- list(
          x = islet$centroid_x_um, 
          y = islet$centroid_y_um, 
          zoom = -1.0
        )
        cat(sprintf("[VITESSCE] Center: (%.1f, %.1f) zoom=%.1f\n", 
                    center$x, center$y, center$zoom))
      }

      # Build title
      title <- sprintf("Case %s", islet$case_id)
      if (!is.null(islet$islet_key)) {
        title <- sprintf("%s - %s", title, islet$islet_key)
      }
      if (!is.null(islet$donor_status)) {
        title <- sprintf("%s (%s)", title, islet$donor_status)
      }
      
      cat(sprintf("[VITESSCE] Building viewer: %s\n", title))
      cat("[VITESSCE RENDER] ===== END =====\n\n")

      # Build and return the vitessce widget
      build_islet_vitessce(
        image_rel = asset$image_rel,
        seg_rel = seg_rel,
        title = title,
        channels = channel_names_vec,
        center = center,
        case_id = islet$case_id,
